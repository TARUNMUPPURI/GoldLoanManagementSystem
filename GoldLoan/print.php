...
if($_SERVER["REQUEST_METHOD"] == "POST")
{
		...
		if(isset($_POST["purity"])){
			...
			else if($_SESSION['purity']==22){
			#before interest
			$r = 2100
			$bint = $weight * $r;
			$_SESSION['bint'] = $bint;
			}
			else if($_SESSION['purity']==24){
			#before interest
			$r = 2300
			$bint = $weight * $r ;
			$_SESSION['bint'] = $bint;
...
$sql = "INSERT INTO `customer`(`firstname`, `lastname`, `mobile`, `email`, `address`, `gender`, `password`, `ornaments`, `weight`, `purity`, `loan`, `interest`, `monthinterest`) 
            VALUES            ('$name','$sname','$mno','$email','$add','$gender','$pass','$ornaments','$weight','$purity','$loan','$interest','$m_int')";
...
<?php
	...
	if (isset($_SESSION['interest'])) {
		echo "Interest for 3month's :- ".$_SESSION['interest']."";
		echo "<br>";
	}
